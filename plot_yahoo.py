...
D1 = Plotting.read_dir("./results/yahoo_T=31278_L=1_e=0.0/")
...
std = True
legendHandles = []
keys = ['epsall_lin', 'mini_lin', 'epsall_gb2', 'mini_gb2', 'epsall_gb5', 'mini_gb5', 'lin', 'rucb_lin', 'rucb_tree', 'rucb_gb5fast']
#'rucb_gb5', 
for k in keys:
    params = []
    mus = []
...
plt.xlabel('Number of interactions (n)', fontsize=FONT_SIZE)
...
plt.sca(ax1)
tt1 = plt.title('Dataset: Yahoo',fontsize=FONT_SIZE)
...
if Args.save:
    plt.savefig("./figs/plots_yahoo.png", format='png', dpi=200, bbox_inches='tight')
    plt.savefig("./figs/plots_yahoo.pdf", format='pdf', dpi=200, bbox_inches='tight')
else:
    plt.show()
